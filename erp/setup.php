...
try {
        ...
        if ($statement !== '') {
            try {
                ...
                if (strpos($e->getMessage(), 'Duplicate') === false && strpos($e->getMessage(), 'already exists') === false) {
                    // ignore missing column errors on fresh install
                }
            }
        }
    }
    ...
    try {
        $pdo->exec("ALTER TABLE products ADD COLUMN is_published TINYINT(1) DEFAULT 1");
    } catch (PDOException $e) {}
    try {
        $pdo->exec("ALTER TABLE deliveries ADD COLUMN order_id INT NULL");
    } catch (PDOException $e) {}
    try {
        $pdo->exec("ALTER TABLE users MODIFY role ENUM('admin','manager','sales','driver','staff') DEFAULT 'staff'");
    } catch (PDOException $e) {}
...
<div class="login-page">
    <div class="login-box">
        ...
        <?php if ($installed): ?>
            ...
            <a href="<?= htmlspecialchars(url('login.php')) ?>" class="btn btn-primary" style="width:100%;text-align:center;margin-top:1rem;display:block;padding:0.75rem">Go to Login</a>
        <?php elseif ($error): ?>
