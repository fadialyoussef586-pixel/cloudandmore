...
$payrollTotal = (float) db()->query("SELECT COALESCE(SUM(net_salary),0) FROM payroll WHERE month = CAST(strftime('%m', 'now') AS INTEGER) AND year = CAST(strftime('%Y', 'now') AS INTEGER)")->fetchColumn();
